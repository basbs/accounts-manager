...
final class ReconcileCommand implements Command {
  ...
  public void run() throws Exception {
    ...
    BigDecimal closingBalance = console.readMoney("Closing balance: ");
    ...
    BigDecimal reconciledBalance = closingBalance;
    ...
    for (UnreconciledTransaction transaction : toBeReconciled) {
      ...
      if (!console.readConfirmation("Does the statement contain this transaction?")) {
        reconciledBalance = reconciledBalance.add(transaction.amount());
    ...
    if (reconciledBalance.equals(totals.checkingBalance())) {
      ...
      console.printf(
          ...
          totals.checkingBalance().toPlainString(),
          reconciledBalance.toPlainString(),
          totals.checkingBalance().subtract(reconciledBalance).toPlainString());
      return;
    }
